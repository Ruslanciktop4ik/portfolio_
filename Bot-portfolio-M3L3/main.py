...
def cansel(message):
    bot.send_message(message.chat.id, "ℹ️ Чтобы посмотреть команды, используй - /info", reply_markup=hideBoard)
  
def no_projects(message):
    bot.send_message(message.chat.id, '⚠️ У тебя пока нет проектов!\n🆕 Можешь добавить их с помощью команды /new_project')
...
attributes_of_projects = {'Имя проекта' : ["✏️ Введите новое имя проекта:", "project_name"],
...
def start_command(message):
    bot.send_message(message.chat.id, "👋 Привет! Я — твой персональный *бот-менеджер проектов*.\n\n"
                                    "🗂 Помогу сохранить все твои проекты и важную информацию о них.\n"
                                    "💡 Чтобы начать, используй команду /info"
"")
    info(message)
...
"📌 *Вот, что я умею:*\n\n"
    ...
    "🔍 Просто введи имя проекта — и я покажу всю информацию о нем!"
...
def link_project(message, data):
    ...
    bot.send_message(message.chat.id, "📊 Выберете текущий статус проекта:", reply_markup=gen_markup(statuses))
...
def callback_project(message, data, statuses):
    ...
    if status not in statuses:
        bot.send_message(message.chat.id, "⚠️ Статус не найден в списке, попробуй еще раз!", reply_markup=gen_markup(statuses))
        bot.register_next_step_handler(message, callback_project, data=data, statuses=statuses)
        return
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, '⚠️ Проект не найден. Выбери проект из списка:', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, '⚠️ Навык не найден. Попробуй еще раз:', reply_markup=gen_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
...
def update_project_step_2(message, projects):
    ...
    if message.text == cancel_button:
        ...
        return
    if project_name not in projects:
        bot.send_message(message.chat.id, "⚠️ Проект не найден. Попробуй снова:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, update_project_step_2, projects=projects )
        return
    bot.send_message(message.chat.id, "🔧 Что хочешь изменить в проекте?", reply_markup=gen_markup(attributes_of_projects.keys()))
...
def update_project_step_3(message, project_name):
    ...
    if attribute not in attributes_of_projects.keys():
        bot.send_message(message.chat.id, "⚠️ Неверный выбор. Попробуй снова:", reply_markup=gen_markup(attributes_of_projects.keys()))
        bot.register_next_step_handler(message, update_project_step_3, project_name=project_name)
        return
...
def update_project_step_4(message, project_name, attribute): 
    ...
    if attribute== "status_id":
        ...
        if update_info in [x[0] for x in rows]:
            ...
            bot.send_message(message.chat.id, "⚠️ Неверный статус. Попробуй снова:", reply_markup=gen_markup([x[0] for x in rows]))
