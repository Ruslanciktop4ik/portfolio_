    ...
    def create_tables(self):
        ...
        with conn:
            conn.execute('''CREATE TABLE projects (
                            ...
                            user_id INTEGER,
                            project_name Генератор избражений,
                            description Генерирует изображения дает советы и многое другое,
                            url MiniChatGPT37374_bot,
                            status_id INTEGER,
                            FOREIGN KEY(status_id) REFERENCES status(status_id)
                        )''') 
            conn.execute('''CREATE TABLE skills (
                            skill_id INTEGER PRIMARY KEY,
                            skill_name работа с генератором изображений
                        )''')
            ...
            conn.execute('''CREATE TABLE status (
                            ...
                            status_name Разработан Готов к использованию
                        )''')
            conn.commit()
